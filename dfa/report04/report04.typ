...
ただし、辺テーブルを作成する際、
$epsilon$ 遷移がある場合には遷移先の頂点も頂点集合に追加されることに注意する.
...
辺テーブルを作成した後は以下の処理を行う.

+ 辺テーブルの行を上から1つずつ見ていき、該当行の頂点集合が構成中のDFAの頂点の頂点集合に一致するものが存在するかどうかを調べる.
	- 存在していない場合、新しく、該当行の頂点集合に対応するDFAの頂点を生成する.
+ 関数 add_dedge() によって、遷移元頂点(上の例の場合、頂点集合が${0 , 1}$であるような頂点)から遷移先頂点(上の例の場合頂点集合が${2, 3}$であるような頂点)へのラベル付の辺を設定する.
+ 辺テーブルのすべての行に対して上の2つの操作を繰り返し行う.

辺テーブルから辺と頂点の設定が終了したら、今まで遷移元の頂点としていた頂点にチェックを入れる.
その後、まだチェック済みでない頂点に対して今までの処理を繰り返す. \
上で述べた通りの処理によってNFAからDFAへの変換が行われることを考察した.
